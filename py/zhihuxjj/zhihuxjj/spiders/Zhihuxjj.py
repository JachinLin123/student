...
class ZhihuxjjSpider(Spider):
    ...
    def start_requests(self):
        ...
        yield Request(self.user_url.format(user=self.start_user,include = self.user_url),callback=self.parse_user) #回调种子用户的个人信息
    ...
    def parse_fo(self, response):
        ...
        for result in results['data']:
            yield Request(self.user_url.format(user=result['url_token'], include=self.user_url),callback=self.parse_user)
